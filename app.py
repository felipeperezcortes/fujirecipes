...
with tab_cc:
    ...
    cc_elegido = st.selectbox(
        "Color Chrome FX", cc_labels, index=0, key="cc_sel")

with tab_ccb:
    _, cb_labels, _ = opciones(blue, "Blue Chrome")
    cb_elegido = st.selectbox(
        "Color Chrome Blue", cb_labels, index=0, key="ccb_sel")
...
if wb_elegido == "Kelvin":
    kelvin = st.number_input(
        "Kelvin (2500–10000)", min_value=2500, max_value=10000, step=100, value=5600)
    col_wb1, col_wb2 = st.columns(2)
    with col_wb1:
        red_offset = st.number_input(
            "Red offset (-4 a +4)", min_value=-4, max_value=4, value=0, step=1)
    with col_wb2:
        blue_offset = st.number_input(
            "Blue offset (-4 a +4)", min_value=-4, max_value=4, value=0, step=1)
